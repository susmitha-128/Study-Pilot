...
- PDF/text extraction (`pdf-parse`) and image reading (Claude vision) → topic tree
...
- Quiz generation, grading (objective + Claude-judged free text), per-topic mastery
...
- DOCX text extraction (currently images go through Claude vision; DOCX needs a
...
`ANTHROPIC_API_KEY` in `.env.local`. This key is only ever read server-side
(`lib/anthropic.ts`) — it is never sent to the browser.
...
lib/
  ...
  anthropic.ts              Server-only Claude client + strict-JSON helper
...
- `SUPABASE_SERVICE_ROLE_KEY` and `ANTHROPIC_API_KEY` are read only in
  server-side code (`lib/supabase/server.ts`, `lib/anthropic.ts`,
