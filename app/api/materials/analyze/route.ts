...
import { askForJSON, askForJSONWithImage } from "@/lib/gemini";

// Gemini calls (especially vision) can run long — give this route room.
...
// Gemini as text. Images (photos of notes/whiteboards) are sent to Gemini
// directly as an image so it can read handwriting/diagrams natively.
...
in minutes and a difficulty (easy|medium|hard).`;
    ...
    if (isImage) {
      ...
      const result = await askForJSONWithImage<{ topics: any[] }>({
        system,
        prompt: `Analyze this photo of study material and extract the topic structure. Return JSON matching: ${jsonShape}`,
        base64,
